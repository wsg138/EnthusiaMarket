...
summarises shape, relationships, and intent. When you change a migration,
...
See [moderation-provider.md](moderation-provider.md) for the operation
lifecycle and operator recovery procedure.
...
Opens when rent fails; `expires_at = started_at + rent.grace-period`.
`cured_at` set when owner tops up before expiry. Expiry without cure ⇒
eviction.
